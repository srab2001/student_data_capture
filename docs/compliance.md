...
| **Production deploy** | ❌ Not live yet | Code is on GitHub (this branch) but not on the production URL. The Vercel project isn't git-linked, so it only deploys via a manual file-upload tool — and that tool has now failed twice on this codebase's size (~150KB/56 files after trimming to just what `next build` needs): the transferred file tree comes back missing most of `lib/`, so the build fails with `Module not found` errors before it ever runs against the database. This reproduced with two different attempts (including one from a fresh session with no memory of the first), so it's a real ceiling on this tool for a codebase this size, not something more retries are expected to fix. **The fix is the one-time manual step already documented in the README's "Deploying" section** (Vercel dashboard → Settings → Git → Connect Git Repository) — after that, deploys clone the repo directly and this class of failure can't happen. |
