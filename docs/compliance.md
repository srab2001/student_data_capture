...
| **Production deploy** | ⚠️ Git-linked, first build still failing | The Vercel project is now connected to this GitHub repo (fixing the manual-upload file-size ceiling recorded below in history). Its first git-triggered build still failed, but with a different, much narrower error: Turbopack reported `Module not found: Can't resolve './globals.css'` from `app/layout.tsx` — a single file, even though `app/globals.css` is confirmed present and correct on GitHub at that exact path. Root cause not yet confirmed; leading theory is a one-time hiccup on the very first build of a freshly-connected repo (e.g. a checkout race), not a repeat of the old file-transfer ceiling. This doc update itself should trigger a fresh auto-deploy to test that theory. |
| **Production deploy — history** | ❌ Manual-upload tool failed twice | Before the git link above, this project deployed via a manual file-upload tool that failed twice on this codebase's size (~150KB/56 files after trimming to just what `next build` needs): the transferred file tree came back missing most of `lib/`, so the build failed with `Module not found` errors before it ever ran against the database. Kept here for the record — the git link above is the fix, not a retry of this path. |
